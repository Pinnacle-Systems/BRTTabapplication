...
import DateTimeInputExample from './utils';
...
const ApprovalPreparation = () => {
  ...
  const [updateLot] = useUpdateApprovalDetailMutation();
...
const ReadonlyField = ({ label, value }) => (
  ...
  const markAsCompleted = async (lotData) => {
    ...
    try {
      ...
      await updateLot({ ...lotData, id: lotData?.[0]?.IID, payload }).unwrap();
...
const renderDetailView = () => {
  ...
  return(
  <div className="fixed inset-0 bg-black bg-opacity-40 z-50 flex justify-center items-start py-20 px-4 overflow-y-auto" onClick={closeDetailView}>
    <div
      className="bg-white rounded-xl shadow-xl w-full max-w-4xl max-h-[calc(100vh-2rem)] flex flex-col" 
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <InfoItem label="Quantity" value={mainItem.BATQTY} />
          <InfoItem label="Process Name" value={mainItem.PROCESSNAME} />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
         <ReadonlyField label="Machine" value={mainItem?.MACHINE} />
          <DropdownField
            label="Loading Contractor"
            value={mainItem?.CONTRACTOR || selectedContractor}
            options={contractDet}
            valueKey="CONAME"
     ...
     {isGrouped && (
      ...
      <div className="sticky bottom-0 bg-white border-t px-4 py-3 flex justify-end">
        <ApproveButton 
          onApprove={() => markAsCompleted(mainItem)} 
...
export default ApprovalPreparation;