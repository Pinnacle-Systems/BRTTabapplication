...
const LoadingPreparation = () => {
  ...
  const { data: prepareLot } = useGetLotPrepareDetailQuery()
  console.log(prepareLot, "prepareLot")
  const prepareLotDet = prepareLot?.data

  console.log(prepareLotDet, "prepareLotDet")
  ...
  const [updateLot] = useUpdateLoadingDetailMutation();
  const { data: ContractorData } = useGetContractorDetailQuery();
  const processName = selectedLot?.[0]?.PROCESSNAME;
  const {
    data: MachineData,
  ...
  useEffect(() => {
    if (selectedLot?.[0]?.MACHINE) {
      setSelectedMachine(selectedLot[0].MACHINE);
    }
  }, [selectedLot]);
  ...
  const markAsCompleted = async (lotData) => {
    if (!selectedMachine || !selectedContractor) {
    ...
    return (
      <div className="detail-view-overlay" onClick={closeDetailView}>
        ...
        >
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0">
                <h3 className="text-base font-semibold text-gray-800 truncate">
                  {mainItem.BATCHNO}
                  {isGrouped && <span className="ml-2 text-sm text-gray-500">({selectedLot.length} items)</span>}
                </h3>
                <p className="text-sm text-gray-500 truncate">{mainItem.CUSTNAME}</p>
              </div>
            </div>
            <button className="text-gray-500 hover:text-red-500 flex-shrink-0" onClick={closeDetailView}>
              <FiX size={20} />
            </button>
          </div>
          ...
          <div className="flex-grow overflow-y-auto p-4 space-y-4">
            {isGrouped && (
              <div className="bg-blue-50 p-3 rounded-md border border-blue-100">
                <h4 className="text-sm font-medium text-blue-800 mb-2">Group Information</h4>
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 text-sm">
                  <div>
                    <span className="text-gray-600">Total Quantity:</span>
                    <span className="ml-1 font-medium">{totalQuantity.toFixed(2)}</span>
                  </div>
                  <div>
                    <span className="text-gray-600">Fabrics:</span>
                    <span className="ml-1 font-medium">
                      {[...new Set(selectedLot.map(item => item.FABRIC))].join(', ')}
                    </span>
                  </div>
                  <div>
                    <span className="text-gray-600">GRN Numbers:</span>
                    <span className="ml-1 font-medium">
                      {[...new Set(selectedLot.map(item => item.GRNNO))].join(', ')}
                    </span>
                  </div>
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-2 gap-3">
              ...
              <InfoItem label="Route" value={mainItem.ROUTE} />
              <InfoItem label="GRN No" value={mainItem.GRNNO} />
              <InfoItem label="Quantity" value={isGrouped ? `${mainItem.BATQTY.toFixed(2)} (of ${totalQuantity})` : mainItem.BATQTY.toFixed(2)} />
              <InfoItem label="From Process" value={mainItem.FROMPROCESS} />
              <InfoItem label="To Process" value={mainItem.PROCESSNAME} />
              <InfoItem label="Party Dc" value={mainItem.PARTYDC} />
              <InfoItem label="prepare By" value={prepareLotDet?.find(value => value.BATCHNO === mainItem.BATCHNO)?.PREPAREDBY || ''} />
            </div>
          ...
          <div className="flex justify-end gap-2 px-4 py-3 border-t bg-gray-50 sticky bottom-0">
            <button className="border text-gray-700 px-3 py-1.5 rounded hover:bg-gray-100 flex items-center text-sm">
              <FiEdit2 className="mr-1" size={16} /> Edit
            </button>
            <button
              className="bg-[#2BA94C] text-white px-3 py-1.5 rounded hover:bg-green-600 flex items-center text-sm"
              onClick={() => markAsCompleted(mainItem)}
            >
              <FiCheckCircle className="mr-1" size={16} /> Complete
            </button>
          </div>
        </div>
      </div>
              ...
              >
                ...
                <div className="card-content">
                  {[
                    ...
                    { label: 'From Process', value: item.FROMPROCESS },
                    { label: 'To Process', value: item.PROCESSNAME, highlight: true },
                    item.MACHINE != null && { label: 'Machine', value: item.MACHINE },
                    { label: 'Party Dc', value: item.PARTYDC },

                    {
                      label: 'lot preparedby',
                      value: prepareLotDet?.find(value => value.BATCHNO === item.BATCHNO)?.PREPAREDBY || ''
                    }
