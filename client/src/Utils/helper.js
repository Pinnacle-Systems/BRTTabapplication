...
export const params = {
  ...
  companyName: secureLocalStorage.getItem(
...
export function getInvoiceTotal(item) {
  ...
  const taxableValue = isNaN(parseFloat(rate * qty))
    ? parseFloat(rate || 0)
    : parseFloat(rate * qty);
  const taxValue = parseFloat(
    (taxableValue / 100) * parseFloat(item?.tax || 0),
  );
  return {
    taxableValue: taxableValue.toFixed(2),
    taxValue: taxValue.toFixed(2),
    totalAmount: (taxableValue + taxValue).toFixed(2),
    splitValue: (taxValue / 2).toFixed(2),
  };
}
...
export const useIdleLogout = (onLogout, isLoggedIn) => {
