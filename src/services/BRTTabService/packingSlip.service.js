...
ROUND(SUM(A.AMOUNT) / SUM(A.STOCKMTRS), 2)  AVG_RATE,A.LOCID,A.STOREID,A.ENDCOUNT,A.WEAVERPCSWNO,A.SETNO,A.BARCODE, B.GTCLOTHCREATIONID AS CLOTHID,ROUND(AVG(A.FABRECRATE),2) AS FABRATE
  ...
  `,
    ...
    for (const item of details || []) {
      ...
      await connection.execute(detailSql, {
        ...
        FABRATE: Number(item.FABRATE),
        AMOUNT: Number(item.AVG_RATE),
      });
    }
