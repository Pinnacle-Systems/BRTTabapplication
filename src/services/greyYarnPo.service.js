...
export async function get(req, res) {
  ...
  try {
    const { gtCompMastId, searchPoNo, searchPosupplier, searchPoDate, searchPoduedate, searchPodelto, isAccepted, deliveryFilter, billEntryFilter } = req.query;
...
WHERE
    ...
    AND gtyarnpo.APPROVALSTATUS = 'APPROVED'
    ...
    ${deliveryFilter ? `and gtyarnpo.docid in (select distinct po.docid
      ...
      ` : ""}
      ${billEntryFilter ? `and gtyarnpo.docid in (select distinct po.docid
        from gtyarnpodet det
        join gtyarnpo po on det.gtyarnpoid = po.gtyarnpoid
       ...
       gtyarnpodet.gtYarnPodetId,
        gtYarnMaster.yarnName,
         gtColorMast.colorName,
          ...
          gtyarnpodet.orderNo,
           gtyarnpodet.noOfBags,
            gtyarnpodet.poQty, gtyarnpodet.bagWt,
             gtyarnpodet.price,gtyarnpodet.amount
         `, { poNo },
    )
    console.log(result)
