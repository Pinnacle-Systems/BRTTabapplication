...
export async function create(req, res) {
    ...
    try {
        ...
        await getNextDyedFabpurInwardNo(connection)
        connection.commit()
...
export async function updateIsRec(req, res) {
    const connection = await getConnection(res)
    try {
        const { gtdfabpurinwarddetId } = req.query;
        const response = await connection.execute(`
        UPDATE gtdfabpurinwarddet a
        SET a.IS_REC = 1
        WHERE a.gtdfabpurinwarddetId = :gtdfabpurinwarddetId
                        `, { gtdfabpurinwarddetId });
...
export async function get(req, res) {
    ...
    JOIN
        gtdfabricpo del on del.gtdfabricpoid = gtdfabricpodet.gtdfabricpoid
    LEFT JOIN 
    ...
    ${isRec ?
            ...
            const resp = result.rows.map(del => ({
                ...
                fabType: del[9], processName: del[10], poId: del[11], orderNo: del[12], grnQty: del[13], poRate: del[14], balQty: del[15], poQty: del[16], grnRolls: del[17], lotNo: del[18], aDelQty: del[19], gtdfabpurinwarddetId: del[20], poDetId: del[21], poNo: del[22], delto: del[23], remarks: del[24], fpino: del[25]
            }))
