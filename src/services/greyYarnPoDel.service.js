...
import { getCurrentFinancialYearId, getCurrentFinancialYearIdAndCode } from "../queries/financialYear.js";
import { STOREID, COMPCODE, COMPNAME, PROJECTID, GY_INWARD_PTRANSACTION, TCODE, TTYPE, LOCID, POTYPE, TRANSTYPE, YARN_STORE_NAME, STOCKTTYPE, COMPSHORTCODE } from "../constants/defaultQueryValues.js"
    ...
    try {
        ...
        await (async function createGridDetails() {
            const promises = deliveryDetails.map(async (deliveryItem) => {
                ...
                select det.yarnname, color, det.buyercode, uom, det.processname, price, tax, orderno, poQty, noofbags, po.gtyarnpoid, po.docdate,
                coalesce(
                (select sum(totalGrnQty) from gtyarnpoinwarddet where gtyarnpoinwarddet.gtYarnPoDetId = det.gtYarnPoDetId)
                , 0)  as agrnQty, gtYarnMaster.yarnname,
                ...
                where det.gtyarnpodetid = ${deliveryItem.poDetId}
                `)
                const [yarnname, color, buyercode, uom, processname, price, tax, orderNo, poQty, poBags, gtyarnpoid, poDate, aGrnQty,
                    yarnAliasName, unitName, colorName, buyerCode, processName, finYearCode
                ...
                const stockSql = `INSERT INTO gtyarnstockmast (GTYARNSTOCKMASTID, TAXRMRATE, RMRATE, PROJECTID, IS_CANCELLED, 
                    ...
                    '${finYearCode}', '${COMPSHORTCODE}', '${YPOINO}','${YPOINDATE}', 'P', '${GY_INWARD_PTRANSACTION}','${orderNo}', 
        ...
        await getNextGreyYarnPoInwardNo(connection)
        connection.commit()
...
export async function get(req, res) {
    ...
    let result;
    try {
        const { gtCompMastId, itemWiseFilter, isReceived
        } = req.query
        let isRec = isReceived
            ? isReceived
                .toString() : ""
        if (itemWiseFilter) {
            const sql = `
...
JOIN 
gtYarnPo deL ON del.GTYARNPOID = gtYarnPoDet.GTYARNPOID
JOIN 
gtCompMast supp ON supp.compName1 = DEL.DELTO 
    where  SUPP.gtCompMastId = :gtCompMastId
    ${isRec ?
                    `AND gtyarnPoInwardDet.IS_REC = ${isRec === 'true' ? 1 : 0}`
                    :
                    ""}
   `
            ...
            const resp = result.rows.map(del => ({
                ...
                delBags: del[8], lotNo: del[9], aDelQty: del[10], gtyarnPoInwardDetId: del[11], poDetId: del[12], poBags: del[14], delTO: del[15], remarks: del[16], vehNo: del[17],
        ...
        gtYarnPo.delTo
 FROM   GTYARNPOINWARD
 JOIN   gtCompMast supp ON supp.compName1 = gtYarnPoInward.supplier
 JOIN gtYarnpoInwardDet ON gtYarnpoInwardDet.gtYarnpoInwardId = gtYarnpoInward.gtYarnpoInwardId
  JOIN gtYarnPo ON gtYarnPoInwardDet.poNo = gtYarnPo.gtYarnPoid
   JOIN   gtCompMast delTo ON delTo.compName1 = gtYarnPo.delTo
JOIN gtYarnPoInwardDet ON gtYarnPoInwardDet.gtYarnPoInwardId = gtYarnPoInward.gtYarnPoInwardId
 WHERE 
...
export async function updateIsRec(req, res) {
    const connection = await getConnection(res)
    try {
        const { gtyarnPoInwardDetId } = req.query;
        const response = await connection.execute(`
        UPDATE gtyarnPoInwardDet a
        SET a.IS_REC = 1
        WHERE a.gtyarnPoInwardDetId = :gtyarnPoInwardDetId
                        `, { gtyarnPoInwardDetId });
...
export async function upDate(req, res) {
    ...
    try {
        ...
        const nonGridResult = await connection.execute(nonGridSql);
        const lastRowData = await connection.execute(`
        select gtyarnpoinwardid, YPOINDATE from gtyarnpoinward where rowid = '${nonGridResult.lastRowid}'
        `)
        const [GTYARNPOINWARDID, YPOINDATE] = lastRowData.rows[0]
        ...
        await (async function updateGridDetails() {
            const promises = deliveryDetails.map(async (deliveryItem) => {
                ...
                let gtYarnPoDetResult = await connection.execute(`
                select det.yarnname, color, det.buyercode, uom, det.processname, price, tax, orderno, poQty, noofbags , po.gtyarnpoid, po.docdate, gtYarnMaster.yarnname,
                gtunitmast.unitname, gtcolormast.colorname, gtbuyermast.buyercode,gtprocessmast.PROCESSNAME, gtFinancialyear.finyr
                from gtyarnpodet det
                join gtYarnPo po on det.GTYARNPOID = po.gtyarnpoid
                join gtYarnMaster on gtYarnMaster.GTYARNMASTERID = det.yarnname
                ...
                where gtyarnpodetid = ${deliveryItem.poDetId}
                    `)
                const [yarnname, color, buyercode, uom, processname, price, tax, orderNo, poQty, poBags, gtyarnpoid, poDate,
                    yarnAliasName, unitName, colorName, buyerCode, processName, finYearCode
                ] = gtYarnPoDetResult.rows[0]
                const taxRate = price + (price / 100 * tax)
                ...
                const stockSql = `INSERT INTO gtyarnstockmast (GTYARNSTOCKMASTID, TAXRMRATE, RMRATE, PROJECTID, IS_CANCELLED, 
                    ...
                    VALUES(supplierseq.nextVal, 0, 0, '${PROJECTID}', 'F', 
                    '${totalGrnQty}', '${excessQty}', '${COMPNAME}', '${deliveryItem.lotNo}', 
                    '${finYearCode}', '${COMPSHORTCODE}', '${yarnPoInwardNo}','${(YPOINDATE ? moment(YPOINDATE).format("DD-MM-YYYY") : '')}', 'P', '${GY_INWARD_PTRANSACTION}','${orderNo}', 
                    '${yarnAliasName}', '${colorName}', '${unitName}', '${totalGrnQty}', ${price}, '${taxRate}' , '${price * totalGrnQty}',
                    '${totalGrnQty * grnQty}', 
                    '${LOCID}', '${YARN_STORE_NAME}', '${processName}','${buyerCode}', '${price}', '${taxRate}', '${TRANSTYPE}', '${STOCKTTYPE}')`;
                await connection.execute(stockSql);
